...
# Feature: Corpus Ingest (Upload)

## Purpose
Add images & PDFs to the search corpus from the browser, with real-time progress tracking. Uploaded assets land under the `corpus/` prefix in B2 and become searchable once the index is built.
...
- Side effects: file stored in B2 bucket under `corpus/{sanitized_filename}`
...
- API validates content type against the allowlist (images: `jpeg/png/gif/webp`; `application/pdf`)
...
- API uses key: `corpus/{sanitized_filename}`
...
- File type not in allowlist (anything other than images / PDFs) → API returns 415
