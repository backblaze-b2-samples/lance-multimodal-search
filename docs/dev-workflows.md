...
- The backend pulls in `sentence-transformers` (CPU `torch`), `lancedb`, `pyarrow`, and `pymupdf`.
- The CLIP model (`clip-ViT-B-32`) downloads lazily from HuggingFace on the **first** search/index call — a one-time, key-free fetch. The first indexing run is slower; later runs are fast.
- To test the core loop locally: upload a few images/PDFs (Upload), build the index (Library), then query by text or image (Search). LanceDB objects appear under the `lancedb/` prefix in your bucket and persist across restarts.
...
- E2E: `pnpm test:e2e` (run `pnpm --filter @lance-multimodal-search/web exec playwright install chromium` once first)
