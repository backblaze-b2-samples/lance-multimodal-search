...
async def lifespan(_app: "FastAPI"):
    ...
    # Ensure the LanceDB table exists and is readable on B2. Empty-schema
    # tables don't persist on S3 backends, so this uses the seed-row create +
    # broken-table recovery (see app/repo/lance_store.py).
    from app.repo import ensure_table_ready

    try:
        ensure_table_ready()
    except Exception:
        logger.warning("LanceDB table init failed at startup", exc_info=True)
    yield
...
app = FastAPI(
    title="Lance Multimodal Search API",
    description=(
        "Object-storage-native multimodal vector search over images and PDFs, "
        "backed by Backblaze B2 + LanceDB + local CLIP embeddings"
    ),
    version="0.1.0",
    lifespan=lifespan,
)
