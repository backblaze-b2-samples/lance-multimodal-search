...
# The corpus is images + PDFs — those are the asset types CLIP can embed
# (PDFs are rendered page-by-page to images at index time). Other file types
# are rejected so the corpus stays focused on searchable media.
ALLOWED_TYPES = {
    "image/jpeg",
    "image/png",
    "image/gif",
...
def process_upload(
    ...
    # version automatically.  No duplicate rejection needed. Source assets land
    # under the corpus/ prefix so the Library view (and indexer) can scope to
    # exactly the user-supplied images & PDFs, distinct from derived/ renders.
    key = f"{settings.corpus_prefix}{safe_name}"
